...
class PostComposerInstall extends Command
{
     ...
     * @var string
     */
    protected $signature = 'ocelot:post-composer-install';
    ...
    {
        echo "Running Ocelot's Post Composer install procedure..." . PHP_EOL;

        // Copy the .env file
        // ------------------
        if (!file_exists('.env')) {
            if (copy('vendor/ocelot/core/.env.example', '.env')) {
